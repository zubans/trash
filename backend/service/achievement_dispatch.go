...
func (d *AchievementDispatcher) eligible(ctx context.Context, order *repository.Order) error {
	...
	// Единственный порог суммы на платформе. Поставляемые скрипты своего числа
	// больше не держат: два порога отказывают по-разному — этот пишет в лог,
	// почему заказ не подошёл, а скрипт молча возвращает None, — и пока оба
	// стояли на одном значении, второй лишь делал первый невидимым. Строже
	// ядра ачивка становится через min_order_amount в своём config.
	//
	// По умолчанию порог нулевой: он отсекал заказы дешевле 300 ₽ при ценах
	// каталога в 100–200 ₽, то есть выключал механизм целиком. Границу
	// проводит не сумма, а то, что ниже неё в этой функции и в §9 документа:
	// заказчик и исполнитель — разные люди, заказ оплачен, суточный потолок
