...
# Модератор вносит паспорт заказчика и фотографирует документ. Сверку данных
# ядро не примет, пока паспорт с фото не дошёл до сервера, — модератор без сети
# сначала досылает паспорт.
