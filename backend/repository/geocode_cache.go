...
type GeocodeCacheRepository interface {
	...
	Lookup(query string) (*CachedGeocode, error)
	// Save upserts the result for a query.
	Save(query, address string, lat, lon float64) error
}
...
func (r *geocodeCacheRepo) Lookup(query string) (*CachedGeocode, error) {
	if r.db == nil {
		return nil, nil
	}
	var c CachedGeocode
...
func (r *geocodeCacheRepo) Save(query, address string, lat, lon float64) error {
	if r.db == nil {
		return nil
	}
	_, err := r.db.Exec(
