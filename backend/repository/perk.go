...
type UserPerk struct {
	...
	// Value — множитель или пункты; у беспроцентного периода nil.
...
//
// Виды передаются вызывающим, а не зашиты здесь: какие виды встают в одну
// очередь, решает сервис (service.CommissionPerkKinds), и SQL очереди не
// должен расходиться с формулой, когда видов станет больше.
type PerkRepository interface {
	...
	Active(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) ([]*UserPerk, error)
	...
	NextStart(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) (time.Time, error)
	// CountQueued — сколько привилегий этих видов ещё не истекло: и
	// действующая, и ждущие в очереди. Это то, что ограничивает
	// max_active_per_user.
	CountQueued(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) (int, error)
	...
	ListQueue(ctx context.Context, userID uuid.UUID, kinds []string, now time.Time) ([]*UserPerk, error)
	...
	DueReminders(ctx context.Context, kinds []string, now, deadline time.Time, limit int) ([]*UserPerk, error)
...
const perkColumns = `p.id, p.user_id, p.kind, p.value, p.starts_at, p.expires_at, p.shop_order_id,
	so.number, p.revoked_at, p.revoked_by, p.granted_by, p.reason, p.created_at`

const perkFrom = ` FROM user_perks p LEFT JOIN shop_orders so ON so.id = p.shop_order_id`

func scanPerk(row rowScanner) (*UserPerk, error) {
	var p UserPerk
	var value sql.NullFloat64
	var number sql.NullInt64
	if err := row.Scan(&p.ID, &p.UserID, &p.Kind, &value, &p.StartsAt, &p.ExpiresAt, &p.ShopOrderID,
...
func (r *perkRepo) Active(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) ([]*UserPerk, error) {
	return r.list(ctx, q, `SELECT `+perkColumns+perkFrom+`
		WHERE p.user_id = $1 AND p.kind = ANY($2) AND p.revoked_at IS NULL
		  AND p.starts_at <= $3 AND p.expires_at > $3
		ORDER BY p.starts_at`, userID, pq.Array(kinds), now)
}
...
func (r *perkRepo) NextStart(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) (time.Time, error) {
	var last sql.NullTime
	err := r.exec(q).QueryRowContext(ctx, `
		SELECT MAX(expires_at) FROM user_perks
		WHERE user_id = $1 AND kind = ANY($2) AND revoked_at IS NULL AND expires_at > $3`,
		userID, pq.Array(kinds), now).Scan(&last)
	if err != nil {
...
func (r *perkRepo) CountQueued(ctx context.Context, q Querier, userID uuid.UUID, kinds []string, now time.Time) (int, error) {
	var count int
	err := r.exec(q).QueryRowContext(ctx, `
		SELECT COUNT(*) FROM user_perks
		WHERE user_id = $1 AND kind = ANY($2) AND revoked_at IS NULL AND expires_at > $3`,
		userID, pq.Array(kinds), now).Scan(&count)
	return count, err
}
...
func (r *perkRepo) Create(ctx context.Context, q Querier, perk *UserPerk) error {
	...
	return r.exec(q).QueryRowContext(ctx, `
		INSERT INTO user_perks (id, user_id, kind, value, starts_at, expires_at, shop_order_id, granted_by, reason)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING created_at`,
		perk.ID, perk.UserID, perk.Kind, perk.Value, perk.StartsAt, perk.ExpiresAt,
...
func (r *perkRepo) ListQueue(ctx context.Context, userID uuid.UUID, kinds []string, now time.Time) ([]*UserPerk, error) {
	return r.list(ctx, nil, `SELECT `+perkColumns+perkFrom+`
		WHERE p.user_id = $1 AND p.kind = ANY($2) AND p.revoked_at IS NULL AND p.expires_at > $3
		ORDER BY p.starts_at`, userID, pq.Array(kinds), now)
}
...
func (r *perkRepo) DueReminders(ctx context.Context, kinds []string, now, deadline time.Time, limit int) ([]*UserPerk, error) {
	if limit <= 0 {
		limit = 100
	}
	return r.list(ctx, nil, `SELECT `+perkColumns+perkFrom+`
		WHERE p.kind = ANY($1) AND p.revoked_at IS NULL AND p.reminded_at IS NULL
		  AND p.expires_at > $2 AND p.expires_at <= $3
		  AND NOT EXISTS (
		      SELECT 1 FROM user_perks n
		      WHERE n.user_id = p.user_id AND n.kind = ANY($1) AND n.revoked_at IS NULL
		        AND n.id <> p.id AND n.expires_at > p.expires_at)
		ORDER BY p.expires_at
		LIMIT $4`, pq.Array(kinds), now, deadline, limit)
}
