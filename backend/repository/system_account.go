...
const (
	...
	// AccountDisputes финансирует выплату исполнителю по спору, который арбитр
	// решил как «неизвестно»: заказчику удержание возвращается целиком, и
	// заплатить исполнителю больше не из чего. Как и BONUSES, уходит в минус, и
	// этот минус — во что обошлись неразрешимые споры.
	AccountDisputes = "DISPUTES"
	// AccountShop собирает выручку магазина. Как и COMMISSION, это собственные
	// деньги платформы, и вывести их может только админ; как и BONUSES, счёт
	// может уйти в минус — возвратом по уже выведенной выручке, и этот минус
	// показывает сверка.
	AccountShop = "SHOP"
)
