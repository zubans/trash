...
type Manifest struct {
	...
	// ManualExecute разрешает исполнителю самому отметить заказ исполненным
	// («Исполнил»). По умолчанию true. Услуга, которую закрывает скрипт по
	// результату проверки, ставит false: тогда кнопки нет, а ядро отклоняет
	// отметку, пришедшую в обход приложения.
	ManualExecute bool `json:"manual_execute"`
	// RequirePassport объявляет, что исполнитель по этой услуге вносит паспорт
	// заказчика с фото. Ядро не примет сверку данных, пока паспорт с фото не
	// дошёл до сервера: заказ не закроется без документа, а скрипту не нужно
	// помнить, пришёл ли он (implementation_plan_delivery_passport.md §3.2).
	RequirePassport bool `json:"require_passport"`
