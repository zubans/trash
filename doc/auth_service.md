...
## Статусы учётной записи и мягкий бан

| Статус | Вход и продление сессии | Что доступно |
| :--- | :--- | :--- |
| `ACTIVE` | да | всё по ролям |
| `SOFT_BANNED` | да | только маршруты белого списка `softBanAllowedRoutes` (`middleware/soft_ban.go`) |
| `BANNED` | нет: `RequireAuth` отвечает `401`, refresh отзывает все сессии | ничего |

`SOFT_BANNED` — явная блокировка аккаунта, при которой человека всё же пускают
в приложение: узнать о блокировке, написать в поддержку и во внутреннюю почту,
увидеть баланс и довести до конца уже взятые заказы (чат заказа, «Исполнил»,
подтверждение, отмена ещё не взятого, чаевые и отзыв, закрытие смены).

- Проверка встроена в `RequireAuth`. Он подключается к группам маршрутов, а
  middleware группы chi выполняет после разбора маршрута, поэтому решение
  принимается по шаблону маршрута (`POST /customer/orders/{id}/confirm`) без
  префикса `/api`.
- Список разрешающий: маршрут, которого в нём нет, для `SOFT_BANNED` закрыт.
  Отказ — `403` с телом `{"error": "account_soft_banned", "message": "..."}`;
  по коду клиент показывает экран блокировки. Не `401`: сессия действительна,
  обновлять её или выкидывать пользователя не нужно.
- Тест `TestSoftBanAllowedRoutes_Exist` проверяет, что каждый маршрут списка
  зарегистрирован в коде: переименованный маршрут иначе молча закрылся бы.
- В проверках допуска (`service/eligibility.go`) `SOFT_BANNED` закрывает новые
