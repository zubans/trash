...
MANIFEST = {
    ...
    "manual_execute": False,          # нет кнопки «Исполнил», ядро отклоняет отметку
...
Исполнитель в каталог заказчика не ходит (`/customer/orders` ему закрыт), поэтому
кнопка «Верифицироваться» в его кабинете размещает тот же заказ через отдельный
эндпоинт — [`service/executor_verification.go`](../backend/service/executor_verification.go):

| Метод | Путь | Что делает |
| :--- | :--- | :--- |
| `GET` | `/api/executor/verification` | `is_verified`, `missing` — незаполненные поля, `address` — адрес по умолчанию, `order` — открытый заказ на верификацию |
| `POST` | `/api/executor/verification` | дозаполняет недостающее и создаёт заказ; не хватает данных — `422 {"error", "missing": [...]}` |
| `POST` | `/api/executor/verification/cancel` | отменяет открытую заявку (claim освобождается, можно заказать снова) |

* Вариант ищется среди включённых: по системному коду `verified` у варианта или
  его категории либо по библиотечному поведению `verification`. Одного поведения
  мало: сохранение узла в админке записывает в него собственную копию скрипта, и
  движок зовёт её по id узла, а не по имени поведения. Не нашлось включённого
  варианта — эндпоинт отвечает «услуга верификации сейчас недоступна» (узлы из
  миграции 043 выключены и имеют другой код).
* Обязательны ФИО, дата рождения (с ними модератор сверяет паспорт) и адрес (туда
  он приедет). При регистрации всё это уже требуется; дозаполнение нужно старым
  аккаунтам. Уже заполненные поля не перезаписываются, и ничего не пишется, пока
  не проверено всё присланное.
* Адрес берётся из профиля (по умолчанию, иначе первый); новый сохраняется
  адресом по умолчанию.
* Дальше это обычный заказ: `can_order`, claim, лента модераторов, проверка
  паспорта и эскалация — всё как у заказчика.

На фронте окно [`VerificationPromptModal`](../frontend/src/pages/executor/components/VerificationPromptModal.vue)
сразу шлёт `POST`; на `422` показывает поля ровно из `missing`, после создания —
состояние «Заявка создана» с адресом и кнопкой отмены. Пока заявка открыта,
напоминание раз в 30 минут не показывается, а в карточке профиля висит значок,
открывающий её.
...
Отметка исполнителя сама по себе личность не подтверждает, а исполненный заказ
заказчик может принять — и заказ закрылся бы без проверки паспорта. Поэтому
манифест верификации объявляет `manual_execute: False` (по умолчанию `True`):

* в карточке заказа `actions.execute = false`, и приложение кнопку не рисует;
* `POST /executor/orders/{id}/execute` отвечает `422`
  (`ErrManualExecuteDisabled`) — для старых сборок и запросов в обход;
* заказ закрывает только скрипт: совпадение данных или решение администратора.

Услуга с `check_fields` не закрывается отметкой исполнителя **никогда**, даже без
флага: это правило держит ядро (`Manifest.ExecutableByHand`), а не скрипт. Иначе
узел со своей копией скрипта, сохранённой в конструкторе до появления флага,
оставил бы дыру открытой — так и было на проде. Флаг `manual_execute: False`
нужен услугам, которые закрывает скрипт, но без сверки данных.
...
На карточке случая два решения, и они разные:

| Кнопка | Что делает |
| --- | --- |
| **Верифицировать** | `POST /api/admin/users/{customer_id}/verified` — та же отметка, что на карточке пользователя. Публикует `user.verified`, по нему скрипт закрывает заказ, платит исполнителю и снимает случай с модерации. Отдельного эндпоинта у кнопки нет намеренно: решение одно и то же, откуда бы его ни приняли. |
| **Снять с модерации** | `POST /api/admin/escalations/{id}/resolve` — ничего не подтверждает. Заказ возвращается исполнителю, и **круг попыток начинается заново**. |

Кнопка «Верифицировать» показывается только при праве `users.edit`: это правка
пользователя, а не разбор модерации, и модератору с одним лишь
`escalations.edit` сервер такой запрос всё равно отклонит.

**Круг попыток.** Скрипту сообщается номер попытки *в текущем круге*, а не
сквозной по заказу. Круг начинается заново после каждого снятия с модерации —
иначе «возврат исполнителю» был бы фикцией: сквозной счётчик уже перевалил за
`MAX_ATTEMPTS`, и первая же следующая отправка уходила бы на модерацию снова.
Сама строка `order_submissions.attempt` остаётся сквозной: её читает
администратор, разбирающий всю историю ввода по заказу.

Третий исход — отменить заказ, если проверка не прошла совсем.
