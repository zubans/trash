...
stateDiagram-v2
    ...
    EXECUTED --> DISPUTED: Заказчик оспорил выполнение
    EXECUTED --> ASSIGNED: Администратор или модератор вернул в работу
    DISPUTED --> COMPLETED: Заказчик подтвердил / арбитр: прав исполнитель или «неизвестно»
    DISPUTED --> CANCELED: Исполнитель признал / арбитр: прав заказчик
...
| `EXECUTED` | На проверке: исполнитель отметил «Исполнил», ждём подтверждения заказчика. Администратор или модератор может вернуть заказ в работу (`POST /api/admin/orders/{id}/return-to-work`). |
| `DISPUTED` | Исполнитель отметил выполнение, заказчик оспорил. Заказ открыт, пока открыт спор (см. раздел 6). |
...
Заказчик может оспорить заказ в `EXECUTED`: `POST /api/customer/orders/{id}/dispute`
с телом `{"claim": "что не выполнено"}`. Заказ переходит в `DISPUTED`, в
`order_disputes` заводится открытый спор, в чат заказа уходит системное
сообщение. Кнопку клиенту даёт `actions.dispute`.

- Оспорить можно только свой заказ, отмеченный исполнителем, и только обычной
  услуги: скриптовые услуги (верификация) закрываются сами, без подтверждения
  заказчика.
- Претензия обязательна, до 2000 символов. Открытый спор на заказ — один.
- Заказчик закрывает спор, подтвердив выполнение обычным
  `POST /api/customer/orders/{id}/confirm`: спор получает закрытие
  `CUSTOMER_CONFIRMED` в той же транзакции, что и выплата.
- Общие пути закрытия заказа (`confirmTx`, `cancelTx`) отказывают заказу с
  открытым спором: закрыть заказ можно только вместе со спором.
- Оспоренный заказ остаётся в списке исполнителя и в «Активных заказах»
...
Уведомления (`service/dispute_notify.go`): об открытии спора — исполнителю, о
закрытии любым способом — обеим сторонам, каждой своими словами. Письмо уходит
во внутреннюю почту (вид `SYSTEM`, `ref_type: order`) и, если e-mail
подтверждён, наружу. Отправка — после коммита действия; e-mail уходит в фоне,
