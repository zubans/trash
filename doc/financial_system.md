...
Ниже — разбивка по счетам (`DEPOSITS`, `ESCROW`, `FINES`, `PAYOUTS`, `COMMISSION`), сопоставление эскроу с суммой активных заказов и списки находок: разошедшиеся балансы и заказы с зависшим удержанием. Списки находок показываются только когда они непустые: пустая таблица читается как «ничего не проверяли», а не как «всё в порядке».
